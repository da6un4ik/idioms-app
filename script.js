const IDIOM_DATA = [
    {
        ...
        exercises: [
            ...
            { id: "ex5", type: "Разговорный тест", dialogue: "— ¿Crees que el test es difícil?", question: "Выберите логичный ответ:", options: ["No, es pan comido.", "Sí,es un test."], answer: "No, es pan comido." }
        ]
    }
];
...
function playAudio(path) {
    ...
    audio.play().catch(e => console.warn("Аудио не найдено:", path));
}
...
function renderDashboard() {
    ...
    dash.innerHTML = `
        <div class="netflix-hero" style="background-image: linear-gradient(to top, #141414, transparent), url('${idiom.image}');">
            <div class="hero-content">
                <h1 style="margin:0 0 15px 0;">${idiom.text} 
                    <span class="audio-btn" onclick="playAudio('${idiom.audio_main}')">🔊</span>
                </h1>
                <button class="check-btn" style="width:auto; padding:10px 25px;" onclick="renderDetail(${idiom.id})">▶ Изучать</button>
            </div>
        </div>
        <p style="padding: 20px; color:#888; font-weight:bold;">ПОПУЛЯРНО СЕЙЧАС</p>
    `;
}
...
function renderDetail(id) {
    ...
    detail.innerHTML = `
        <button onclick="showScreen('screen-dashboard')" style="background:none; color:white; font-size:24px; border:none; margin: 10px 0 20px 0; cursor:pointer;">← Назад</button>
        ...
        <div style="background:#333; padding:15px; border-radius:4px; margin-bottom:30px; border-left: 4px solid #E50914;">
            <strong>Пример:</strong><br> ${idiom.example} 
            <span class="audio-btn" onclick="playAudio('${idiom.audio_example}')">🔊</span>
        </div>
...
function renderExercise(ex) {
    ...
    if (ex.type === "Выбор значения" || ex.type === "Разговорный тест") {
        const diag = ex.dialogue ? `<p style="color:#aaa; border-left:2px solid #555; padding-left:12px; font-style:italic; margin-bottom:12px;">${ex.dialogue}</p>` : '';
        ...
        content = `<input type="text" id="in-${ex.id}" placeholder="Введите слово..." style="width:100%; padding:14px; background:#141414; color:#fff; border:1px solid #444; border-radius:4px;">`;
    ...
    return `<div class="exercise-block" id="block-${ex.id}"><h4>${ex.type}</h4><p style="margin-bottom:15px;">${ex.question}</p>${content}<div id="feed-${ex.id}"></div><button class="check-btn" onclick="checkAnswer('${ex.id}')">ПРОВЕРИТЬ</button></div>`;
}
...
function checkAnswer(exId) {
    ...
    feed.innerHTML = correct ? '<span class="correct">✅ Правильно!</span>' : '<span style="color:#E50914; display:block; margin-top:10px;">❌ Попробуй еще раз</span>';
}
